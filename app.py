...
# 精确坐标转换库（需要先安装：pip install coord-convert）
...
def generate_equidistant_waypoints(path, num_segments=6):
    """
    将路径 path (list of [lng,lat]) 按长度均匀分成 num_segments 段
    返回 num_segments+1 个航点 (包含起点和终点)
    """
...
class HeartbeatSim:
    def __init__(self, start_point):
        self.current_pos = start_point[:]          # 当前经纬度 [lng, lat]
        self.waypoints = []                        # 等分航点列表 (含起点终点)
        self.current_wp_idx = 0                    # 当前正在前往的目标航点索引 (下一个要到达的航点)
        self.running = False
        ...
        self.speed_pct = 50
        self.altitude = 50
        self.total_segments = 0                    # 总段数 = 航点数-1
        self.arrival_flag = False                  # 到达新航点标志 (用于页面提示)
        self.arrived_wp_index = -1                 # 刚到达的航点索引
    ...
    def set_path(self, waypoints, altitude, speed_pct):
        ...
        self.current_wp_idx = 1                    # 下一个目标航点是索引1
        self.running = True
        self.finished = False
    ...
    def update_one_step(self):
        """每次调用模拟一个时间步长 (HEARTBEAT_INTERVAL)，移动无人机并检测到达航点"""
...
def main():
    ...
    # ==================== 飞行监控页面 ====================
        ...
        if st.session_state.flight_started and not st.session_state.flight_paused and st.session_state.sim and st.session_state.sim.running:
            ...
            for _ in range(steps):
                ...
                if new_hb:
                    ...
                    break
            # 检测到达新航点并显示消息
            if st.session_state.sim.arrival_flag:
        ...
        progress = st.session_state.sim.progress if hasattr(st.session_state.sim, 'progress') else 0
        # 计算当前航点序号
        current_wp = st.session_state.sim.current_wp_idx  # 下一个目标航点索引
        total_wp = len(st.session_state.sim.waypoints)
        # 当前已到达的航点索引 = current_wp - 1 (已经到达的最后一个)
        ...
        remaining_dist = (1 - progress) * path_length(st.session_state.sim.waypoints) * 111000  # 粗略米
        ...
        with col_right:
            ...
            if st.session_state.waypoints:
                for i, wp in enumerate(st.session_state.waypoints):
                    folium.CircleMarker([wp[1], wp[0]], radius=4, color='blue', fill=True,
